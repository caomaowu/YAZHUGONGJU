...
export function computeBarrelInnerLength(rawSpecs) {
...
export function normalizeMachineRawSpecs(rawSpecs) {
  if (!rawSpecs) return rawSpecs;

  const barrelInnerLength = computeBarrelInnerLength(rawSpecs);

  if (barrelInnerLength === undefined) return rawSpecs;
